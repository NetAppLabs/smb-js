...
test.serial('should be granted read permission when querying on file', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('annar') as SmbFileHandle;
...
test.serial('should return non-locked writable when creating writable and not keeping existing data', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-overwrite', {create: true}) as SmbFileHandle;
...
test.serial('should return non-locked writable when creating writable and keeping existing data', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-append', {create: true}) as SmbFileHandle;
...
test.serial('should return error when writing unsupported type', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-unsupported-type', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing blob', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-blob', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing typed array', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-typed-array', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing data view', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-data-view', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing array buffer', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-array-buffer', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should return error when writing unsupported object type', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-unsupported-object-type', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should return error when writing unsupported object data type object', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-unsupported-object-data-type-object', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should return error when writing unsupported object data type', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-unsupported-object-data-type', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing blob via struct', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-blob-via-struct', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  const blob = new Blob([JSON.stringify({hello: 'world'}, null, 2)], {type: 'application/json'});
...
test.serial('should succeed when writing typed array via struct', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-typed-array-via-struct', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing data view via struct', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-data-view-via-struct', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing array buffer via struct', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-array-buffer-via-struct', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when not keeping existing data and writing string', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-string', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when not keeping existing data and writing string via struct', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-string-via-struct', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when keeping existing data and writing string', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-append-string', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when keeping existing data and writing string via struct', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-append-string-via-struct', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing string multiple times', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-strings', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when writing string multiple times via struct', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-strings-via-struct', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when seeking past size of file', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-seek-past-size', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should succeed when seeking past size of file via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-seek-past-size-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when seeking position', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-seek', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when seeking position via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-seek-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when writing string after seek', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-string-after-seek', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
  await t.notThrowsAsync(writable.seek(6));
...
test.serial('should succeed when writing string after seek via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-string-after-seek-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
  await t.notThrowsAsync(writable.write({type: 'seek', position: 6}));
  await writable.write('there');
...
test.serial('should succeed when seeking past size of file and writing string via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-seek-past-size-and-write-string-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when seeking and writing string via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-seek-and-write-string-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when seeking and writing string object via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-seek-and-write-string-object-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when truncating size', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-truncate', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when truncating size via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-truncate-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when writing string after truncating size', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-string-after-truncate', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when writing string after truncating size via write', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-write-string-after-truncate-via-write', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  await writable.write('hello rust');
...
test.serial('should succeed when aborting writable file stream', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-abort', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
...
test.serial('should return writer for writable file stream', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-writer', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  t.false(writable.locked);
...
test.serial('should return error when getting writer for locked writable file stream', async (t) => {
  const rootHandle = await getRootHandle();
  const fileHandle = await rootHandle.getFileHandle('writable-writer-locked', {create: true}) as SmbFileHandle;
  const writable = await fileHandle.createWritable();
  t.false(writable.locked);
  const writer = writable.getWriter();
  await new Promise(r => setTimeout(r, 10)); // XXX: writable.locked is set by write stream sink's start method which gets invoked asynchronously
